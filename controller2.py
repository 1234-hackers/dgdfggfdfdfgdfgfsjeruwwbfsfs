...
def  reset_pass():
    if request.method == "POST":
        ...
        if existing:
            ...
            msg = Message('Hello Use This Code To Verify Email', recipients =  [email])
            msg.html =  render_template("email_template.html" , code = code,mess = msg)
            Post_guy.send(msg)
            ...
            return redirect(url_for('register', mess = "You Are Not Registered Please Create An Account"))
...
class Base_form(FlaskForm):
    ...
    class Meta:
        csrf = False
        csrf_class = SessionCSRF 
        csrf_secret = "dfgdfgtryfhgfhhgfhdxfdffjsllflcxvmcxmzcnsndcxzoweoerohd"
        csrf_time_limit = timedelta(minutes=25)
    ...
    if request.method == "POST" and "img" in request.files:
        ...
        if passc == passc2  and not registered:
            ...
            if users.find_one({"email":email}):
                code = random.randint(145346 , 976578)
                code = str(code)
                session['veri_user'] = email
                crete_ver =Message("Hello , You Got This Code" + code + "To Verify The Email That Will Be Linked With Your  Account",
                        sender = service_mail ,
                        recipients = [email]
                             )
                Post_guy.send(crete_ver)
...
def main_page():
    ...
    the_user = user_db.find_one({"email" : user_email})
    favs = the_user["favs"]
    fav_arr = []
    if len(favs) <10:
        count = 2
...
@application.route('/main2/' , methods = ['POST','GET'])
def main2():
...
@application.route('/change_email/' , methods = ['POST','GET'])
@csrf.exempt
def change_email():
    user = mongo.db.users
    user_email = session['login_user']
    if request.method == "POST":
        ...
        passc = request.form['passcode']
        real_user = user.find_one({"email" : user_email})
        real_pass = real_user['password']
        if Hash_passcode.verify(real_pass , passc):
            verif = mongo.db.verify_email
            code = random.randint(145346 , 976578)
            code = str(code)
            #send the code here
            verc = Message(
                "Hello , You Got This Code" + code + "To Verify New Email That Will Be Linked With Your  Account Enter The Following Code To Verify Email" ,
                sender = service_mail ,
            ...
            verif.insert_one({"email" : new_email , "code" :code}) 
...
@application.route('/verify_email/' , methods = ['POST','GET'])
@csrf.exempt
def verify_email():
    user = mongo.db.users
    verif = mongo.db.verify_email
    ...
    if request.method == "POST":
        email = request.form['email']
        code = request.form['ver_code']
        exists = verif.find_one({'email' : email})
        if exists:
            the_code = exists['code']
            if the_code ==code:
                user.find_one_and_update({"email" : user_email} , {"set" : { "email" : email}})
...
def post_on_tags():
    ...
    return render_template('post_on_tags.html.html')
...
def post():
    if request.method == "POST":
       ...
       # sec3 = request.form['sec3']
        ...
        if allowed_file(filename):
            my_dir = upload_folder
            pic.save(os.path.join(my_dir, filename))
            image = upload_folder + "/" + filename
            namez2 =  upload_folder   +"/" + post_id +".jpg"
            if "/" in post_id :
                post_id = post_id.replace("/" , "vb")
            os.rename(image  , upload_folder +"/" + post_id +".jpg")
            
        link_db.insert_one({"owner" : owner , "link" : link , "secondary" : sec_arr, "likes" : like_arr , "comments" : comments ,
                            "tags" : tag_arr , "title" : title , "description" : desc , "class" : post_class , "post_id" : post_id , "direc" : namez2  })
        return redirect(url_for('main_page'))
